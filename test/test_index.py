...
@pytest.mark.parametrize(
    "currently_edited_file, target_import_path, symbol,  expected_import", [
        ('package_a/module_b.py', 'package_a/module_a.py', 'AnotherStuff', 'from package_a.module_b import AnotherStuff'),
    ],
)
def test_index_absolute_import(currently_edited_file, target_import_path, symbol, expected_import):
    with FakeProject() as fp:
        index = Index()
        index.index_project(root=fp.project_root)
        candidates = index.get_candidates(fp.project_root, symbol)
...
def test_index_relative_import(currently_edited_file, target_import_path, symbol, expected_import):
    ...
    with FakeProject() as fp:
        index = Index()
        index.index_project(root=fp.project_root)
        candidates = index.get_candidates(fp.project_root, symbol, current_file_path=currently_edited_file)
        assert expected_import in candidates
